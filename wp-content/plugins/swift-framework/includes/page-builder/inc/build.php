    ...
    */
    ...
    class SFPageBuilderSetup extends SFPageBuilderAbstract {
        ...
        public function spb_register_frontend_css() {
            ...
            $spb_dynamic_scripts = false;
            
            if ( isset( $sf_opts['enable_min_styles'] ) ) {
                $enable_min_styles = $sf_opts['enable_min_styles'];
            }
            if ( isset( $sf_opts['spb_dynamic_scripts'] ) ) {
                $spb_dynamic_scripts = $sf_opts['spb_dynamic_scripts'];
            }

            if ( $spb_dynamic_scripts ) {
                global $post;
                $spb_active = sf_get_post_meta($post->ID, '_spb_js_status', true);
                if ( $spb_active != "true" ) {
                    return;
                }
            }
